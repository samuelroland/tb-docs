...
Lors des rares sessions d'exercice en classe et durant la révision en dehors des cours, un temps important est perdu à mettre en place les exercices et les vérifications manuelles du comportement du programme développé.
...
Faire fonctionner le programme n'est que la première étape. Faire du code robuste, modulaire, lisible et performant demande des retours humains pour pouvoir progresser. Par manque de retour sur leurs réponses, les étudiant·es moins expérimenté·es se rendront compte si la compétence derrière l'exerice était vraiment acquise, seulement durant les évaluations plusieurs semaines plus tard.

Côté de l'enseignante, en comptant uniquement sur les questions des étudiant·es, savoir si le message de l'exercice est passé reste un challenge. Il est difficile aussi de savoir quand l'exercice doit se terminer. Peut-être qu'il aurait fallu 5 minutes de plus pour qu'une majorité ait le temps de finir ? Pour avoir accès aux réponses, elles doivent être manuellement rendues sur un serveur. Ce rendu prend à nouveau du temps pour chaque étudiant·e. Pour l'enseignante, les récupérer, ouvrir et fermer 40 fichiers, pour évaluer à l'oeil l'état d'avancement et comprendre les différentes approches, prendrait trop de temps en classe.

Une autre approche serait de coder dans un fichier Google Docs partagé à toute la classe. L'enseignante a maintenant un moyen de relire au fur et à mesure, détecter les incompréhensions, mais les étudiant·es ont perdu toute l'expérience du développement en local. Dans Google Docs, il n'y a plus de couleurs, plus d'auto-complétion, plus d'erreurs de compilation intégrée dans le code. Tous les raccourcis, le formattage automatique et les informations au survol manquent terriblement. Pour tester leur programme, les étudiant·es doivent constamment copier leur code dans un fichier local.

En conclusion, le problème c'est que l'entrainement est fastidieux pour les étudiants, ce qui ralentit l'apprentissage en profondeur de la programmation. Leurs enseignant·es n'ont pas accès à leurs réponses, n'ont pas de vision précise des incompréhensions et ne peuvent pas donner de feedbacks.
...
#figure(
  ...
  caption: [Aperçu d'un exercice de C dans PLX, avec un check qui échoue et les 2 suivants qui passent @PlxDocsStatus],
...
== Nouveaux défis
Le besoin de feedbacks humain pour les étudiant·es et celui de permettre aux enseignant·es d'accéder aux réponses, ne sont pas encore résolus par PLX. Ces nouveaux défis sont à la base des deux extensions majeures qui seront développées dans le cadre de ce travail.
...
Comme l'application fonctionne localement et s'exécute à chaque sauvegarde, le code et les résultats des checks sont déjà connus par PLX. Il suffirait d'avoir un serveur central, utilisé durant les sessions d'entrainement en classe (nommées "les sessions live"), vers lequel PLX envoie le code et l'état des checks en continu. Ces informations peuvent ensuite être récupérées du serveur et affichées sur l'interface PLX de l'enseignant·e.

Ces interfaces de visualisation sur une échelle globale ou plus zoomée, permettra aux enseignant·es de rapidement comprendre les lacunes des étudiant·es, en relisant les différentes réponses affichées. Grâce à l'état des checks, il sera facile de voir si la classe a terminé l'exercice. Il sera possible de sélectionner certaines réponses particulières pour les afficher au beamer, les commenter spécifiquement ou apporter des feedbacks à l'oral.

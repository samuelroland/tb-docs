...
Les clients leader ont besoin d'identifier la source du message transféré. Quand un nouveau bout de code arrive dans l'interface, soit le bout de code se met en bas de la liste pour un nouveau client, soit il met à jour le code existant du client existant. Le `client_id` ne peut pas être utilisé car il doit rester secret, il ne doit pas être envoyé vers un autre client. Nous avons besoin d'un autre identifiant, différent à chaque session pour un client donné pour garder l'intérêt de l'anonymat.
...
En cas de redémarrage, les participant·es devraient recréer ou rejoindre les nouvelles sessions à la main. Le tableau de bord des leaders prendraient quelques minutes à retrouver son état précédent en attendant que tous les partipant·es aient sauvés et envoyés leur fichier une première fois.
